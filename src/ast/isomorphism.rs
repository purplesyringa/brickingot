...
pub fn compare<T: ?Sized + Isomorphic>(arena: &Arena<'_>, x: &T, y: &T) -> bool {
    Checker {
        arena,
        group_maps: FxHashMap::default(),
        var_maps: FxHashMap::default(), // FIXME: populate
    }
    .compare(x, y)
}

pub struct Checker<'arena, 'code> {
...
impl Isomorphic for Variable {
    fn compare(&self, other: &Self, checker: &mut Checker<'_, '_>) -> bool {
        self.version == other.version || checker.var_maps.get(&self.version) == Some(&other.version)
    }
}
// fn compare_stmts(&mut self, x: &StmtMeta<AnalysisIr>, y: &StmtMeta<AnalysisIr>) -> bool {
//     x.meta.measure == y.meta.measure
// }

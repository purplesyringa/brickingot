use super::insn_control_flow::{InsnControlFlow, can_insn_throw, get_insn_control_flow};
use super::insn_stack_effect::get_insn_stack_effect;
use super::{BasicBlock, BytecodePreparsingError};
use crate::ClassInfo;
use crate::ast::Str;
...
pub fn extract_basic_blocks<'code>(
    ...
    // at EH boundaries. Also at the end and the beginning for implementation simplicity. Save all
    // non-trivial transitions to build the CFG without reparsing the instructions.
    ...
    // Splitting BBs at exception handling boundaries allows the CFG to operate on BB level at all
    // times. It is not spelled entirely clearly in the specification, but it seems like valid JVM
    // bytecode never has `start_pc`/`end_pc` point into the middle of instructions, and HotSpot
    // also verifies that's the case, so we're in good company.
    ...
    // code can confuse future passes, so it makes sense to do this unconditionally, especially
    // because even `javac` can emit unreachable bytecode, e.g. for
    //     try {
    //         return;
    //     } finally {
    //         finally_body();
    ...
    // `nop` works). This means that we cannot automatically assume all exception handlers are
    // reachable. This interpretation is reasonable for future IRs, with the exception of no-op
    ...
    // are in range `[target; end)`, and none of them throw, recursing into the `catch` handler
    // would leave unreachable code in the IR. This cannot be fixed during post-processing, so we
    // need to integrate this logic into DFS. This requires differentiating between throwing BBs,
    ...
    // completely consistent with the exception ranges this pass outputs.

    let [mut remaining_eh, mut remaining_eh_throwing] = [
        // Applies to all BBs.
        |handler: &ExceptionHandler<'_>| {
            Some(handler.active_range.start..handler.active_range.end.min(handler.target))
        },
        // Applies only to throwing BBs.
        |handler: &ExceptionHandler<'_>| {
            if handler.target < handler.active_range.end {
                Some(handler.active_range.start.max(handler.target)..handler.active_range.end)
            } else {
                None
            }
        },
    ...
    for handler in &mut exception_handlers {
        ...
        // We've already removed unreachable `try` regions.
        assert!(start < end, "unreachable try region");
